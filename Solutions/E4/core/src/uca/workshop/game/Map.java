...
public class Map { // Clase que representa el nivel de nuestro juego con todos los objetos de el.
	...
	private Array<Enemy> enemies; // Un array que contiene los enemigos del juego
	...
	private void update() { // Actualiza los valores de los personajes del juego
		...
		for(Enemy enemy : enemies) {
			enemy.update();
			if(player.getBody().overlaps(enemy.getBody())) // Si colisiona con un enemigo
				reset();
		}
	...
	private void processMapMetadata() { // Este metodo inicializa los elementos del mapa.
		...
		objects = map.getLayers().get("Enemies").getObjects(); // Cogemos del mapa los objetos llamados "Enemies"
		
		for (MapObject object : objects) {
			RectangleMapObject rectangleObject = (RectangleMapObject) object; // Lo transformamos en rectangulo
			Rectangle rectangle = rectangleObject.getRectangle();
			Enemy enemy = new Enemy(); // Lo transformamos en enemigo
			enemy.setPosition(rectangle.x, rectangle.y); // Le ponemos la posicion indicada
			enemies.add(enemy); // Las añadimos al array
		}
